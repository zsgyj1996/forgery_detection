...
from utils.image_util import resize_max_res, chw2hwc, colorize_forgery_maps, BayarConv2d_half, rgb2gray, ResNet50
...
class ForgeryEstimationPipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        self.bayarConv = BayarConv2d_half(in_channels=1, out_channels=3, padding=2)

        # Convert the BayarConv weights to half precision
        # pretrained_bayarConv = {'kernel': pretrained_dict['constrain_conv.kernel'].half()}
        ...
        self.bayar_extractor = ResNet50(n_input=3).half()
        pretrained_extractor = {k[16:]: v.half() for k, v in pretrained_dict.items() if 'noise_extractor.' in k}
        ...
        # self.bayar_extractor.load_state_dict(pretrained_extractor, strict=True)
        self.bayar_backbone = ResNet50(n_input=3).half()
        ...
        # self.bayarConv = self.bayarConv.half().to(self.device)
        # self.bayar_extractor = self.bayar_extractor.half().to(self.device)
        # self.bayar_backbone = self.bayar_backbone.half().to(self.device)
        ...
        rgb_norm = rgb_norm.half()  # convert to fp16
    ...
    def single_infer(self, rgb_in: torch.Tensor, num_inference_steps: int, show_pbar: bool):
        ...
        forgery_latent = forgery_latent.half()
    ...
    def encode_bayar(self, rgb_in: torch.Tensor) -> torch.Tensor:
        ...
        #bayar = bayar.to(self.bayarConv.parameters().__next__().dtype)
    ...
    def encode_backbone(self, rgb_in: torch.Tensor) -> torch.Tensor:
        ...
        #backbone_latent = backbone_latent.to(self.bayar_backbone.parameters().__next__().dtype)
